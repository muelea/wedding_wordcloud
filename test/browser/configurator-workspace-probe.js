  ...
  run.addEventListener('click', async () => {
    ...
    try {
      ...
      for (const button of actions) {
        ...
        check(`${button.id}: named SVG button with touch target`, button.getAttribute('aria-label') &&
          button.querySelector('svg[aria-hidden="true"] use') && rect.width >= 44 && rect.height >= 44);
      }
