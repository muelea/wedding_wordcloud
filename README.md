...
### Local Printful mockups

To compare a fixed cart design with Printful's generated product photography,
start the local application with the explicit operator flag:

```bash
./run_local.sh --printful-mockups
```

Each saved design in the configurator cart then has a **Printful-Mockup**
button. It submits that exact immutable design to Printful's asynchronous v2
Mockup Generator, shows the returned views in the cart and provides a download
for each PNG. It requests 2000 × 2000 px first. Some new-store allowances
reject that resolution even when the request counter is otherwise unused; in
that specific case the tool automatically retries once at 1000 × 1000 px and
labels the result accordingly. This creates no order and does not use or relax
either Printful fulfillment-write switch.

The control and its API are available only for that process, only when
`APP_ENVIRONMENT=local`, and only through `localhost`, `127.0.0.1` or `::1` on
the same machine. They remain absent from normal local runs and return 404 in
hosted/production environments. Do not use the LAN URL printed at startup for
this operator tool.

The mockup flag can be combined with the marketing seed in either order:

```bash
./run_local.sh --seed-cloud marketing/clouds/classic-wedding.json --printful-mockups
```

For this combination, the seed command prints a `localhost` event URL labeled
**Printful-Mockups** first; open that URL on the computer running the server.
It also prints the LAN URL for phones and other devices, but that URL
intentionally does not expose the mockup control.

Printful cannot fetch a browser-only localhost URL. The server therefore
renders each exact SVG print surface to a same-aspect PNG (bounded to 3000 px
on its longest side), uploads it under a separate temporary prefix in the
existing private Supabase Storage bucket and gives Printful a one-hour signed
HTTPS URL. The PNG avoids indefinitely pending provider tasks observed with
SVG layers; it is only a temporary mockup source and does not replace the
immutable SVG used for fulfillment. Sources are deleted as soon as the task
...
The local gate enforces the lower Printful allowance of two creation requests
per minute. Printful currently documents 2 requests/minute for new stores, 10
for stores with at least USD 10 in fulfilled orders, and 20,000 generated files
per environment per day. One valid variant/style/placement combination counts
as one generated file. The command requires the existing `PRINTFUL_API_KEY`,
`PRINTFUL_STORE_ID`, `SUPABASE_URL` and `SUPABASE_SECRET_KEY` values in the
ignored local `.env`; `PRINTFUL_MOCKUP_TOOLS_ENABLED` is deliberately a
process-only value owned by `run_local.sh`, not an `.env` setting.

### Local marketing word clouds

To create a fully functional event with many weighted words before starting the
local server, pass a versioned JSON seed to the same startup command:
...
use the same importer directly:

```bash
npm run local:seed-cloud -- marketing/clouds/classic-wedding.json
```

Each file contains event metadata plus one object per word. `count` represents
the number of simulated guest contributions, not a direct font size:

```json
{
  "version": 1,
  "event": {
    "title": "Marketing – Klassische Hochzeit",
    "locale": "de",
    "pin": "2468"
  },
  "words": [
...
organizer removal, reset, export and the product configurator all retain their
normal behavior. Counts around 18–25 work well for a leading word, 8–14 for
important words, 3–7 for supporting words and 1–2 for the background; the
display's sizing curve intentionally levels off at high counts.

The importer refuses production/hosted application environments and, by
default, any database that is not on localhost. If `DATABASE_URL` deliberately
targets a remote development database, acknowledge that once in the ignored
local `.env`:

```dotenv
ALLOW_REMOTE_MARKETING_SEEDS=true
```

After that, the ordinary `run_local.sh --seed-cloud path/to/cloud.json` and
`npm run local:seed-cloud -- path/to/cloud.json` commands need no additional
flag. Never configure this setting in Fly.
...
| `ALLOW_REMOTE_MARKETING_SEEDS` | local setting | One-time acknowledgement that the local marketing importer may write to the remote development `DATABASE_URL`; forbidden in the production web process. |
